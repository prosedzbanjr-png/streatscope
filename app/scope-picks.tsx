...
export function ScopePicks(){
 ...
 return <section className="scope-picks"><header><div><p>SCOPE PICKS / REDAKCJA</p><h2>THIS WEEK&apos;S<br/><em>BEST.</em></h2></div><span>WYBRANE PRZEZ STREETSCOPE</span></header><div className="scope-picks-grid">{place&&<a href={`/guide/${place.id}`} className="scope-pick-card"><img src={optimizedImageUrl(place.image_url||"/images/hero.png",1080)} alt=""/><div/><article><small>📍 PLACE OF THE WEEK</small><h3>{place.name}</h3><p>{place.short_description||"StreetScope poleca."}</p></article></a>}{fashion&&<a href={`/fashion/${fashion.id}`} className="scope-pick-card"><img src={optimizedImageUrl(fashion.image_url||"/images/hero.png",1080)} alt=""/><div/><article><small>👕 {fashion.badge||"FIT OF THE WEEK"}</small><h3>{fashion.title}</h3><p>{fashion.score_overall!=null?`SCOPE SCORE ${fashion.score_overall.toFixed(1)}/10`:fashion.subtitle}</p></article></a>}{motor&&<a href={`/motor/${motor.id}`} className="scope-pick-card"><img src={optimizedImageUrl(motor.image_url||"/images/hero.png",1080)} alt=""/><div/><article><small>🏎️ {motor.badge||"BUILD OF THE WEEK"}</small><h3>{motor.title}</h3><p>{motor.score_overall!=null?`SCOPE SCORE ${motor.score_overall.toFixed(1)}/10`:motor.subtitle}</p></article></a>}</div></section>;
}
