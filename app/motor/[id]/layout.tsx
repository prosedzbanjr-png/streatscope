...
async function getFeature(id:string):Promise<FeatureMeta|null>{
  ...
  try{
    const query=new URLSearchParams({id:`eq.${id}`,kind:"eq.motor",published:"eq.true",archived_at:"is.null",select:"title,subtitle,description,image_url,gallery",limit:"1"});
...
export async function generateMetadata({params}:{params:Promise<{id:string}>}):Promise<Metadata>{
  ...
  if(!feature)return {title:"Build niedostępny | Motor",robots:{index:false,follow:false}};
  const description=feature.subtitle||feature.description||"Street Motor w StreetScope.";
  const image=feature.image_url||feature.gallery?.find(Boolean)||"/images/hero.png";
  const canonical=`/motor/${id}`;
...
export default function MotorDetailLayout({children}:Readonly<{children:React.ReactNode}>){return children;}
