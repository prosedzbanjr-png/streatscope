...
export function PublishedStories() {
  ...
  useEffect(() => {
    getSupabase().from("articles").select("id,title,category,excerpt,image_url,pinned,published_at").eq("status", "published").is("archived_at", null).lte("published_at", new Date().toISOString()).order("pinned", { ascending: false }).order("published_at", { ascending: false }).limit(3).then(({ data }) => {
      const items = data ?? [];
