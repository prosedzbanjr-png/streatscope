...
export default function MarketPage(){
 ...
 useEffect(()=>{(async()=>{const {data}=await getSupabase().from("market_vehicles").select("id,brand,model,year,price,mileage,image_url,status,featured,drivetrain,transmission,sale_mode,auction_start_price,auction_min_increment,auction_current_bid,auction_bid_count,auction_ends_at").eq("sale_mode","sale").order("featured",{ascending:false}).order("created_at",{ascending:false});setRows((data as Vehicle[]|null)??[]);})()},[]);
