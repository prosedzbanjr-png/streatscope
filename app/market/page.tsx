...
export default function MarketPage(){
 ...
 const load=async()=>{const {data}=await getSupabase().from("market_vehicles").select("id,brand,model,year,price,mileage,image_url,status,featured,drivetrain,transmission,sale_mode,auction_start_price,auction_min_increment,auction_current_bid,auction_bid_count,auction_ends_at").eq("sale_mode","sale").order("featured",{ascending:false}).order("created_at",{ascending:false});setRows((data as Vehicle[]|null)??[])};
 useEffect(()=>{void load();const sb=getSupabase();let active=true;const refresh=()=>{if(active)void load()};const channel=sb.channel("market-live-list").on("postgres_changes",{event:"*",schema:"public",table:"market_vehicles"},refresh).subscribe();const interval=window.setInterval(()=>{if(document.visibilityState==="visible")refresh()},2000);const onVisibility=()=>{if(document.visibilityState==="visible")refresh()};document.addEventListener("visibilitychange",onVisibility);return()=>{active=false;window.clearInterval(interval);document.removeEventListener("visibilitychange",onVisibility);void sb.removeChannel(channel)}},[]);
