...
export default function AuctionsPage(){
 ...
 useEffect(()=>{(async()=>{const {data}=await getSupabase().from("market_vehicles").select("id,brand,model,year,mileage,image_url,status,featured,auction_start_price,auction_min_increment,auction_current_bid,auction_bid_count,auction_ends_at").eq("sale_mode","auction").order("featured",{ascending:false}).order("auction_ends_at",{ascending:true});setRows((data as Vehicle[]|null)??[]);})()},[]);
 ...
 return <main className="market">
  <header className="market-nav"><a href="/" className="market-logo">STREET<span>SCOPE</span></a><nav><a href="/market">MARKET</a><a className="active" href="/market/licytacje">LICYTACJE</a></nav></header>
  <section className="market-hero market-hero-split market-hero-bg"><div className="market-hero-copy"><p>TOW & TRADE · LICYTACJE</p><h1>LICY<br/><em>TACJE.</em></h1><span>Dealer wystawia. Miasto podbija. Najwyższa oferta zgarnia wóz.</span></div><div className="market-hero-logo-bg" aria-hidden="true"/></section>
  ...
  <section className="market-content-split market-content-bg"><div className="market-grid market-grid-main">{filtered.map(v=>{const ended=!v.auction_ends_at||new Date(v.auction_ends_at)<=new Date();const current=v.auction_current_bid??v.auction_start_price??0;return <a className={`vehicle-card auction-card ${ended?"sold":""}`} href={`/market/${v.id}`} key={v.id}><div className="vehicle-photo" style={v.image_url?{backgroundImage:`url(${v.image_url})`}:undefined}>{v.featured&&<b>WYRÓŻNIONE</b>}<span>{ended?"ZAKOŃCZONA":"LICYTACJA"}</span></div><div className="vehicle-info"><small>{v.year||"—"}{v.mileage!=null?` · ${v.mileage.toLocaleString("pl-PL")} MI`:""}</small><h2>{v.brand} {v.model}</h2><p>{v.auction_bid_count||0} ofert · minimalne podbicie $${Number(v.auction_min_increment||500).toLocaleString("en-US")}</p><strong>{v.auction_current_bid?"AKTUALNA OFERTA":"CENA STARTOWA"} · $${Number(current).toLocaleString("en-US")}</strong><small className="auction-end">{ended?"LICYTACJA ZAKOŃCZONA":`KONIEC: ${new Date(v.auction_ends_at!).toLocaleString("pl-PL")}`}</small></div></a>})}{filtered.length===0&&<div className="market-empty">BRAK AKTYWNYCH LICYTACJI.</div>}</div><aside className="market-yard-bg"><div><small>TOW & TRADE</small><h3>LITTLE BIGHORN AVE</h3><p>Licytuj online albo wpadnij na plac i sprawdź pozostałe samochody dostępne u dealera.</p></div></aside></section>
