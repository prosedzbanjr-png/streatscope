...
export default function StatystykiPage() {
  ...
  useEffect(() => { const client = getSupabase(); client.auth.getUser().then(async ({ data }) => { const email = data.user?.email?.toLowerCase() || ""; const {data:person}=await client.from("staff_accounts").select("role,active").eq("email",email).maybeSingle(); const ok = Boolean(person?.active && ["editor_in_chief","deputy_editor_in_chief"].includes(person.role)); setAllowed(ok); if (!ok) return; const { data: rows } = await client.from("articles").select("id,title,category,status,published_at,views").is("archived_at",null).order("views", { ascending: false }); setArticles((rows as Article[] | null) ?? []); }); }, []);
  ...
  return <main className="stats-page"><header><a className="wordmark" href="/">STREET<span>SCOPE</span></a><a href="/redakcja">← PANEL REDAKCJI</a></header><section className="stats-head"><p className="kicker"><i /> ANALITYKA REDAKCYJNA</p><h1>ZASIĘG<br /><em>MATERIAŁÓW.</em></h1><p>Każde wejście w opublikowany materiał zwiększa licznik odsłon.</p></section><section className="stat-cards"><article><span>ŁĄCZNE ODSŁONY</span><b>{totalViews}</b></article><article><span>OPUBLIKOWANE / 7 DNI</span><b>{weekCount}</b></article><article><span>WSZYSTKIE PUBLIKACJE</span><b>{published.length}</b></article></section><section className="ranking"><div><p className="kicker"><i /> TOP MATERIAŁY</p><h2>CO LUDZIE<br /><em>CZYTAJĄ.</em></h2></div><ol>{top.length ? top.map((article, index) => <li key={article.id}><span>0{index + 1}</span><div><b>{article.category}</b><h3>{article.title}</h3></div><strong>{article.views ?? 0}<small>ODSŁON</small></strong></li>) : <p>Jeszcze nie ma opublikowanych materiałów.</p>}</ol></section></main>;
}
