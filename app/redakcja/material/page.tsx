...
type HomepageSlot = "auto" | "hero" | "card1" | "card2" | "card3" | "card4" | "brief1" | "brief2" | "brief3" | "brief4" | "brief5";

const homepagePlacementOptions: Array<{value:HomepageSlot;label:string;description:string}> = [
  {value:"auto",label:"AUTOMATYCZNIE",description:"Materiał ustawi się według daty publikacji."},
  {value:"hero",label:"DUŻY KAFEL / HERO",description:"Najważniejszy materiał na samej górze."},
  {value:"card1",label:"KAFEL 1",description:"Pierwsza karta w sekcji Najnowsze."},
  {value:"card2",label:"KAFEL 2",description:"Druga karta w sekcji Najnowsze."},
  {value:"card3",label:"KAFEL 3",description:"Trzecia karta w sekcji Najnowsze."},
  {value:"card4",label:"KAFEL 4",description:"Czwarta karta w sekcji Najnowsze."},
  {value:"brief1",label:"SKRÓT 1",description:"Pierwsza pozycja w skrócie redakcyjnym."},
  {value:"brief2",label:"SKRÓT 2",description:"Druga pozycja w skrócie redakcyjnym."},
  {value:"brief3",label:"SKRÓT 3",description:"Trzecia pozycja w skrócie redakcyjnym."},
  {value:"brief4",label:"SKRÓT 4",description:"Czwarta pozycja w skrócie redakcyjnym."},
  {value:"brief5",label:"SKRÓT 5",description:"Piąta pozycja w skrócie redakcyjnym."}
];
...
export default function MaterialPage() {
  ...
  function openPlacement(event: {preventDefault():void}) {
    event.preventDefault(); const currentBody = canvas.current ? serializeCanvas(canvas.current) : bodyRef.current; const text = cleanText(currentBody);
  ...
  async function assignHomepagePlacement(publishedId:number, placement:HomepageSlot) {
    const {data}=await client().auth.getSession(); const token=data.session?.access_token;
    if(!token)throw new Error("Sesja wygasła.");
    const response=await fetch("/api/redakcja/home-layout",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({mode:"assign",source_type:"article",source_id:publishedId,slot:placement==="auto"?null:placement})});
    const result=await response.json().catch(()=>({}));
    if(!response.ok)throw new Error(result.error||"Nie udało się ustawić miejsca na głównej.");
  }
  async function save(event: {preventDefault():void}, action: "draft" | "review" | "published", placement:HomepageSlot="auto") {
    ...
    try {
      ...
      if (result.data?.id) { await client().from("article_versions").insert({ article_id: result.data.id, title: payload.title, excerpt: payload.excerpt, body: payload.body, image_url: payload.image_url, created_by: currentEmail }); if(status==="published"){try{await assignHomepagePlacement(result.data.id,placement);}catch(error){placementWarning=error instanceof Error?error.message:"Nie udało się ustawić miejsca na głównej.";}} if (nextReviewStatus === "review") await notifyDiscord("review", result.data.id); const isScheduled = status === "published" && Boolean(planned && new Date(planned).getTime() > Date.now()); if (status === "published" && !isScheduled) await notifyDiscord("published", result.data.id); const activityAction = status === "published" ? (isScheduled ? "article_scheduled" : "article_published") : nextReviewStatus === "review" ? "article_submitted" : isEditing ? "article_updated" : "article_created"; await logActivity({actorEmail:currentEmail,action:activityAction,entityType:"article",entityId:result.data.id,entityLabel:payload.title}); }
      ...
      setPlacementOpen(false); setMessage(placementWarning ? `Materiał zapisany, ale ${placementWarning}` : status === "published" ? (planned && new Date(planned).getTime() > Date.now() ? "Materiał zatwierdzony i zaplanowany. Wybrane miejsce zajmie o wskazanej porze." : isEditing ? "Materiał zaktualizowany i ustawiony na głównej." : "Materiał opublikowany i ustawiony na głównej.") : nextReviewStatus === "review" ? "Materiał przekazany do akceptacji." : "Szkic zapisany."); await loadQueue();
  ...
  async function deleteArticle(id: number, articleTitle: string) {
    if (!canManageArticles) { setMessage("Tylko Redaktor Naczelny albo Zastępca może archiwizować materiały."); return; }
    if (!window.confirm(`Przenieść materiał „${articleTitle}” do archiwum? Będzie można go później przywrócić.`)) return;
    setBusy(true);
    const { error } = await client().from("articles").update({ archived_at: new Date().toISOString(), archived_by: currentEmail, updated_at: new Date().toISOString() }).eq("id", id);
    setBusy(false);
    if (error) { setMessage(error.message); return; }
    await logActivity({actorEmail:currentEmail,action:"article_archived",entityType:"article",entityId:id,entityLabel:articleTitle});
    setMessage("Materiał przeniesiony do archiwum.");
  ...
  useEffect(() => {
    ...
    queue.querySelectorAll<HTMLAnchorElement>('a[href*="/redakcja/material?id="]').forEach(link => {
      ...
      button.textContent = "ARCHIWIZUJ";
      button.title = "Przenieś materiał do archiwum";
