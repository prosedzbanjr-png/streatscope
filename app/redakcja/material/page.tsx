...
export default function MaterialPage() {
  ...
  async function save(event: FormEvent, action: "draft" | "review" | "published") {
    ...
    try {
      ...
      if (error) throw error;
      if (result.data?.id) { await client().from("article_versions").insert({ article_id: result.data.id, title: payload.title, excerpt: payload.excerpt, body: payload.body, image_url: payload.image_url, created_by: currentEmail }); if (nextReviewStatus === "review") await notifyDiscord("review", result.data.id); if (status === "published") await notifyDiscord("published", result.data.id); const activityAction = status === "published" ? "article_published" : nextReviewStatus === "review" ? "article_submitted" : isEditing ? "article_updated" : "article_created"; await logActivity({actorEmail:currentEmail,action:activityAction,entityType:"article",entityId:result.data.id,entityLabel:payload.title}); }
