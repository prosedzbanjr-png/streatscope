...
export default function MaterialPage() {
  ...
  const canvas = useRef<HTMLDivElement>(null); const inlineFile = useRef<HTMLInputElement>(null);
  ...
  function command(name: string, value?: string) { canvas.current?.focus(); document.execCommand(name, false, value); syncBody(); }
  ...
  async function insertImage(event: ChangeEvent<HTMLInputElement>) {
    ...
    try { const url = await upload(file); command("insertImage", url); } catch { setMessage("Nie udało się przesłać zdjęcia do treści."); } finally { event.target.value = ""; }
  }
  ...
  return <main className="material-page"><header><a className="wordmark" href="/">STREET<span>SCOPE</span></a><nav><a href="/redakcja">PANEL</a><a href="/redakcja/statystyki">STATYSTYKI</a></nav></header><form className="material-layout"><section className="material-intro"><p className="kicker"><i /> EDYTOR REDAKCYJNY</p><h1>NAPISZ<br /><em>MATERIAŁ.</em></h1><p>Formatowanie tekstu, cytaty, listy, linki oraz obrazy w środku artykułu.</p></section><section className="material-form"><label>TYTUŁ<input value={title} onChange={event => setTitle(event.target.value)} placeholder="Co wydarzyło się w mieście?" maxLength={120} /></label><div className="material-row"><label>KATEGORIA<select value={category} onChange={event => setCategory(event.target.value)}><option>AKTUALNOŚCI</option><option>ULICE</option><option>SPORT</option><option>OPINIE</option><option>WYDARZENIA</option></select></label><label>ZDJĘCIE GŁÓWNE<input type="file" accept="image/jpeg,image/png,image/webp" onChange={event => setCover(event.target.files?.[0] ?? null)} /><small>{cover ? cover.name : "JPG, PNG lub WEBP · maks. 8 MB"}</small></label></div><label>ZAJAWKA<input value={excerpt} onChange={event => setExcerpt(event.target.value)} placeholder="Krótki opis widoczny na stronie głównej…" maxLength={320} /></label><label>PEŁNA TREŚĆ<div className="material-editor"><div className="material-toolbar" role="toolbar" aria-label="Formatowanie"><button type="button" onClick={() => command("bold")}><b>B</b></button><button type="button" onClick={() => command("italic")}><i>I</i></button><button type="button" onClick={() => command("underline")}><u>U</u></button><button type="button" onClick={() => command("strikeThrough")}><s>S</s></button><span /><button type="button" onClick={() => command("formatBlock", "h2")}>NAGŁÓWEK</button><button type="button" onClick={() => command("formatBlock", "h3")}>ŚRÓDTYTUŁ</button><button type="button" onClick={() => command("formatBlock", "blockquote")}>CYTAT</button><span /><button type="button" onClick={() => command("insertUnorderedList")}>• LISTA</button><button type="button" onClick={() => command("insertOrderedList")}>1. LISTA</button><button type="button" onClick={addLink}>LINK</button><button type="button" onClick={() => inlineFile.current?.click()}>+ ZDJĘCIE</button></div><div ref={canvas} className="material-canvas" contentEditable suppressContentEditableWarning data-placeholder="Napisz materiał. Zaznacz tekst i użyj paska formatowania." onInput={syncBody} /></div></label><input ref={inlineFile} className="visually-hidden" type="file" accept="image/jpeg,image/png,image/webp" onChange={insertImage} /><div className="material-actions"><small>{message}</small><button type="button" disabled={busy} onClick={event => save(event, "draft")}>{busy ? "ZAPIS…" : "ZAPISZ SZKIC"}</button><button type="button" className="primary" disabled={busy} onClick={event => save(event, "published")}>{busy ? "PUBLIKACJA…" : "OPUBLIKUJ ↗"}</button></div></section></form></main>;
}
