...
export default function MaterialyPage(){
  ...
  async function load(email:string, chief:boolean){
    const [articles,features,guides]=await Promise.all([
      client().from("articles").select("id,title,excerpt,status,review_status,updated_at,archived_at,author_email").order("updated_at",{ascending:false}),
      client().from("street_features").select("id,kind,title,subtitle,published,review_status,updated_at,archived_at,created_by,submitted_by").order("updated_at",{ascending:false}),
      client().from("guide_places").select("id,name,category,neighborhood,short_description,active,review_status,updated_at,submitted_by,archived_at").order("updated_at",{ascending:false})
    ]);
    ...
    for(const row of (articles.data??[]) as any[]){
      if(!chief && row.author_email?.toLowerCase()!==email) continue;
    ...
    for(const row of (features.data??[]) as any[]){
      if(!chief && ![row.created_by,row.submitted_by].some((v:string|null)=>v?.toLowerCase()===email)) continue;
    ...
    for(const row of (guides.data??[]) as any[]){
      if(!chief && row.submitted_by?.toLowerCase()!==email) continue;
      const s:Status=row.archived_at?"archived":row.review_status==="review"?"review":row.review_status==="changes_requested"?"changes_requested":row.active?"published":"hidden";
  ...
  async function archive(item:Item){
    if(!confirm(`Przenieść „${item.title}” do archiwum?`))return;
    const now=new Date().toISOString();
    const res=item.kind==="article"?await client().from("articles").update({archived_at:now,updated_at:now}).eq("id",item.id):item.kind==="guide"?await client().from("guide_places").update({active:false,archived_at:now,updated_at:now}).eq("id",item.id):await client().from("street_features").update({archived_at:now,updated_at:now}).eq("id",item.id);
  ...
  return <main className="materials-page">
    ...
    <section className="materials-list">{filtered.length?filtered.map(item=><article key={item.key}><div className="materials-type"><b>{kindLabel[item.kind]}</b><span className={`status status-${item.status}`}>{statusLabel[item.status]}</span></div><div className="materials-info"><h2>{item.title}</h2><p>{item.subtitle||"Brak zajawki."}</p><small>AKTUALIZACJA: {new Date(item.updatedAt).toLocaleString("pl-PL")}</small></div><div className="materials-actions"><a href={item.edit}>EDYTUJ</a>{item.status==="published"&&<a href={item.preview} target="_blank">PODGLĄD ↗</a>}{isChief&&item.status!=="archived"&&<button onClick={()=>archive(item)}>ARCHIWIZUJ</button>}</div></article>):<p className="materials-empty">Brak materiałów pasujących do filtrów.</p>}</section>
  </main>;
}
