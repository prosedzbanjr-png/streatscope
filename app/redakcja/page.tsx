...
type Article = { id: number; title: string; category: string; excerpt: string; body: string | null; image_url: string | null; gallery: string[] | null; status: "draft" | "published"; created_at: string; published_at: string | null; pinned?: boolean; views?: number };
...
export default function RedakcjaPage() {
  ...
  const [articleQuery, setArticleQuery] = useState(""); const [articleFilter, setArticleFilter] = useState<"all" | "published" | "draft" | "scheduled">("all");
  ...
  const recentPublished = published.filter(article => article.published_at && new Date(article.published_at).getTime() >= weekAgo).length;
  const nowTime = Date.now();
  const visibleArticles = articles.filter(article => {
    const matchesQuery = `${article.title} ${article.category}`.toLowerCase().includes(articleQuery.trim().toLowerCase());
    const isScheduled = article.status === "published" && !!article.published_at && new Date(article.published_at).getTime() > nowTime;
    const matchesFilter = articleFilter === "all" || (articleFilter === "published" && article.status === "published" && !isScheduled) || (articleFilter === "draft" && article.status === "draft") || (articleFilter === "scheduled" && isScheduled);
    return matchesQuery && matchesFilter;
  });

  async function loadData() {
    const [articleResult, tipResult] = await Promise.all([
      client().from("articles").select("id,title,category,excerpt,body,image_url,gallery,status,created_at,published_at,pinned,views").order("pinned", { ascending: false }).order("created_at", { ascending: false }).limit(60),
