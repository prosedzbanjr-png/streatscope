...
export default function DashboardPage(){
  ...
  useEffect(()=>{ client().auth.getUser().then(async({data})=>{
    ...
    const [articleReview,featureReview,guideReview,tips,recruitment,archivedArticles,archivedFeatures,archivedGuides,archivedTips,staff,publishedArticles,publishedFeatures,publishedGuides,scheduled,recentLogs]=await Promise.all([
    ...
    const all=[articleReview,featureReview,guideReview,tips,recruitment,archivedArticles,archivedFeatures,archivedGuides,archivedTips,staff,publishedArticles,publishedFeatures,publishedGuides,scheduled,recentLogs];
    const errors=all.map(x=>x.error).filter(Boolean);if(errors.length)setMessage(errors[0]?.message||"Nie udało się pobrać części danych.");
    setCounts({
      review:(articleReview.count||0)+(featureReview.count||0)+(guideReview.count||0),
      tips:tips.count||0,
      recruitment:recruitment.count||0,
      archivedMaterials:(archivedArticles.count||0)+(archivedFeatures.count||0)+(archivedGuides.count||0),
      archivedTips:archivedTips.count||0,
      staff:staff.count||0,
      published:(publishedArticles.count||0)+(publishedFeatures.count||0)+(publishedGuides.count||0),
  ...
  return <main className="chief-dashboard">
    <header><a href="/" className="wordmark">STREET<span>SCOPE</span></a><nav><a href="/redakcja/materialy">MATERIAŁY</a><a href="/redakcja/material">EDYTOR</a><a href="/redakcja/zarzadzaj">ZARZĄDZAJ</a><a href="/redakcja/guide">SCOPE GUIDE</a><a href="/redakcja/logi">LOGI</a></nav></header>
    ...
    <section className="dashboard-cards"><a href="/redakcja/materialy"><span>WSZYSTKIE MATERIAŁY</span><b>ALL</b><em>ARTYKUŁY · LOOK · BUILD · GUIDE →</em></a><a href="/redakcja/zarzadzaj"><span>DO AKCEPTACJI</span><b>{counts.review}</b><em>OTWÓRZ KOLEJKĘ →</em></a><a href="/redakcja/zarzadzaj"><span>AKTYWNE ZGŁOSZENIA</span><b>{counts.tips}</b><em>PRZEJRZYJ →</em></a><a href="/redakcja/rekrutacja"><span>NOWE REKRUTACJE</span><b>{counts.recruitment}</b><em>PRZEJRZYJ →</em></a><a href="/redakcja/guide"><span>SCOPE GUIDE</span><b>LS</b><em>MIEJSCA & PROMO →</em></a><a href="/redakcja/statystyki"><span>OPUBLIKOWANE</span><b>{counts.published}</b><em>STATYSTYKI →</em></a><a href="/redakcja/zarzadzaj"><span>ZAPLANOWANE</span><b>{counts.scheduled}</b><em>HARMONOGRAM →</em></a><a href="/redakcja/zespol"><span>AKTYWNY ZESPÓŁ</span><b>{counts.staff}</b><em>ZARZĄDZAJ →</em></a><a href="/redakcja/archiwum"><span>ARCHIWUM</span><b>{counts.archivedMaterials+counts.archivedTips}</b><em>{counts.archivedMaterials} materiałów · {counts.archivedTips} zgłoszeń →</em></a></section>
    <section className="dashboard-bottom"><div><p className="kicker"><i/> OSTATNIA AKTYWNOŚĆ</p>{logs.length?<div className="recent-logs">{logs.map(row=><article key={row.id}><span>{actionLabel[row.action]||row.action}</span><b>{row.entity_label||row.entity_type}</b><small>{row.actor_email} · {new Date(row.created_at).toLocaleString("pl-PL")}</small></article>)}</div>:<p>Brak zapisanych działań.</p>}<a className="logs-link" href="/redakcja/logi">PEŁNY DZIENNIK DZIAŁAŃ →</a></div><aside><p className="kicker"><i/> SKRÓTY</p><a href="/redakcja/materialy">WSZYSTKIE MATERIAŁY</a><a href="/redakcja/material">+ NOWY ARTYKUŁ</a><a href="/redakcja/kultura">FASHION / MOTOR</a><a href="/redakcja/guide">SCOPE GUIDE / PROMO</a><a href="/redakcja/rekrutacja">REKRUTACJA</a><a href="/redakcja/archiwum">ARCHIWUM</a><a href="/redakcja/zespol">ZESPÓŁ</a><a href="/redakcja/statystyki">STATYSTYKI</a></aside></section>
  </main>;
}
