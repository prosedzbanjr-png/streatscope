...
type Counts = { review:number; tips:number; recruitment:number; archivedArticles:number; archivedTips:number; staff:number; published:number };
...
export default function DashboardPage(){
  ...
  useEffect(()=>{ client().auth.getUser().then(async({data})=>{
    ...
    const [review,tips,recruitment,archivedArticles,archivedTips,staff,published,recentLogs]=await Promise.all([
      ...
      client().from("articles").select("id",{count:"exact",head:true}).eq("status","published").is("archived_at",null),
    ...
    const errors=[review.error,tips.error,recruitment.error,archivedArticles.error,archivedTips.error,staff.error,published.error,recentLogs.error].filter(Boolean);
    if(errors.length)setMessage(errors[0]?.message||"Nie udało się pobrać części danych.");
    setCounts({review:review.count||0,tips:tips.count||0,recruitment:recruitment.count||0,archivedArticles:archivedArticles.count||0,archivedTips:archivedTips.count||0,staff:staff.count||0,published:published.count||0});
